  ...
  this.init = () => {
    ...
    this.traverse((child) => {
      if (child instanceof THREE.Mesh) {
        ...
        child.geometry = tempGeometry; // eslint-disable-line
...
TransformGizmo.prototype.update = function update(rotation, eye) {
...
function TransformGizmoRotate() {
  ...
  this.setActivePlane = function setActivePlane(axis) {
  ...
  this.update = function update(rotation, eye2) {
...
function TransformGizmoScale() {
  ...
  this.setActivePlane = function setActivePlane(axis, eye) {
    const tempMatrix = new THREE.Matrix4();
    eye.applyMatrix4(tempMatrix.getInverse(tempMatrix.extractRotation(this.planes.XY.matrixWorld)));
...
function TransformControls(camera, domElement) {
  ...
  this.setTranslationSnap = function setTranslationSnap(translationSnap) {
    scope.translationSnap = translationSnap;
  };

  this.setRotationSnap = function setRotationSnap(rotationSnap) {
  ...
  this.setSpace = function setSpace(space) {
    scope.space = space;
    this.update();
    scope.dispatchEvent(changeEvent);
  };
